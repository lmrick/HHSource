package com.sulake.habbo.window.utils.floorplaneditor
{
   ...
   public class HeightMapEditor
   {
      ...
      public static var floor_editor_tile_base:Class = class_3511;
      
      public static var floor_editor_tile_entry:Class = floor_editor_tile_entry_png;
      
      public static var floor_editor_tile_base_large:Class = floor_editor_tile_base_large_png;
      
      public static var floor_editor_tile_entry_large:Class = floor_editor_tile_entry_large_png;
      ...
      private var _bitmapElement:class_3282;
      ...
      public function HeightMapEditor(param1:com.sulake.habbo.window.utils.floorplaneditor.BCFloorPlanEditor)
      {
         var _loc2_:int = 0;
      ...
      private function applyDraw(param1:int, param2:int) : void
      {
         ...
         switch(_bcFloorPlanEditor.drawMode)
         {
            ...
            case _bcFloorPlanEditor.drawModes[2]:
               ...
               if(_loc3_ >= 0)
               {
                  _bcFloorPlanEditor.floorPlanCache.setHeightAt(param1,param2,Math.min(29,_loc3_ + 1));
                  break;
               }
               break;
      ...
      private function updateView() : void
      {
         ...
         _loc13_ = 0;
         while(_loc13_ < _floorPlan.floorHeight)
         {
            _loc12_ = 0;
            while(_loc12_ < _floorPlan.floorWidth)
            {
               _loc7_ = transformToScreenSpace(_loc12_,_loc13_);
               _loc10_ = Math.min(_loc10_,_loc7_.x);
               _loc9_ = Math.min(_loc9_,_loc7_.y);
               _loc2_ = Math.max(_loc2_,_loc7_.x);
               _loc1_ = Math.max(_loc1_,_loc7_.y);
               if(_floorPlan.isEntryPoint(_loc12_,_loc13_))
               {
                  _loc5_ = getEntryTile();
                  _loc8_.push({
                     "point":_loc7_,
               ...
               else if((_loc14_ = Math.min(_floorPlan.getHeightAt(_loc12_,_loc13_),29)) >= 0)
               {
                  _loc5_ = getColoredTile(_loc14_,_floorPlan.isTileReserved(_loc12_,_loc13_));
                  _loc8_.push({
                     "point":_loc7_,
                     "image":_loc5_
                  });
         ...
         var _loc11_:BitmapData = new BitmapData(_loc2_ - _loc10_ + 18,_loc1_ - _loc9_ + 27,false,0);
         var _loc3_:Point = new Point(-_loc10_,-_loc9_);
         for each(var _loc4_ in _loc8_)
         {
            _loc11_.copyPixels(_loc4_.image,_loc4_.image.rect,_loc4_.point.add(_loc3_));
         }
         _bcFloorPlanEditor.heightMapBitmapElement.bitmap = _loc11_;
      }
