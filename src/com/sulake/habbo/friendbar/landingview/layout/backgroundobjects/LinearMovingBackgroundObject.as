package com.sulake.habbo.friendbar.landingview.layout.backgroundobjects
{
   import com.sulake.core.window.class_3151;
   import com.sulake.habbo.friendbar.landingview.HabboLandingView;
   import com.sulake.habbo.friendbar.landingview.layout.backgroundobjects.events.PathResetEvent;
   import flash.events.EventDispatcher;
   
   public class LinearMovingBackgroundObject extends BackgroundObject
   {
       
      
      private var var_3492:int;
      
      private var var_3515:int;
      
      private var var_2529:Number;
      
      private var var_2473:Number;
      
      private var var_3019:Number;
      
      private var var_2931:Number;
      
      public function LinearMovingBackgroundObject(param1:int, param2:class_3151, param3:EventDispatcher, param4:HabboLandingView, param5:String)
      {
         super(param1,param2,param3,param4,param5);
         var _loc7_:Array;
         var _loc6_:String = String((_loc7_ = param5.split(";"))[0]);
         var_3492 = int(_loc7_[2]);
         var_3515 = int(_loc7_[3]);
         var_3019 = Number(_loc7_[4]);
         var_2931 = Number(_loc7_[5]);
         var_2529 = var_3492;
         var_2473 = var_3515;
         sprite.assetUri = "${image.library.url}reception/" + _loc6_ + ".png";
      }
      
      override public function update(param1:uint) : void
      {
         super.update(param1);
         if(!sprite)
         {
            return;
         }
         var _loc3_:int = 0;
         var _loc2_:int = 0;
         var_2529 += param1 * var_3019;
         var_2473 += param1 * var_2931;
         sprite.x = var_2529;
         sprite.y = var_2473 + window.desktop.height;
         if(var_3019 > 0 && sprite.x > _loc3_ || var_3019 < 0 && false || var_2931 > 0 && sprite.y > _loc2_ || var_2931 < 0 && false)
         {
            var_2529 = var_3492;
            var_2473 = var_3515;
            events.dispatchEvent(new PathResetEvent("LWMOPRE_MOVING_OBJECT_PATH_RESET",id));
         }
      }
   }
}
